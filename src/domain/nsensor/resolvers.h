...
public:
  ...
  String resolve(JsonObject reqJson) override {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      dataObj["isAlive"] = millis() - nodes[i].lastSeen < 10000;

      JsonObject sensorObj = dataObj.createNestedObject("sensors");
      for(int j = 0; j < sizeof(sensorSchema.names) / sizeof(sensorSchema.names[0]); j++) {
...
public:
  ...
  String resolve(JsonObject reqJson) override {
    ...
    data["isAlive"] = millis() - nodes[index].lastSeen < 10000;

    JsonObject sensorObj = data.createNestedObject("sensors");
    for(int i = 0; i < sizeof(sensorSchema.names) / sizeof(sensorSchema.names[0]); i++) {
