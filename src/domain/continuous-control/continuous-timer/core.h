...
#ifndef SG_MCU_CONTINUOUS_TIMER_CORE_H
#define SG_MCU_CONTINUOUS_TIMER_CORE_H

class ContinuousTimerState : public ContinuousState {
public:
...
class ContinuousTimerCore : public ContinuousControl {
public:
  explicit ContinuousTimerCore(ContinuousGpioChain *gpioChain) : ContinuousControl(CON_CTRL_TIMER, gpioChain) {
    ContinuousTimerModel model;
    timer = model.get().timer;
    rtcCore = RtcCore::instance();
  }

  ~ContinuousTimerCore() override = default;

  ContinuousTimerState getControlState() {
    return state;
  }
...
private:
  ...
  ContinuousTimerState state;
  ContinuousTimerSchema::Timer timer;
};

#endif //SG_MCU_CONTINUOUS_TIMER_CORE_H
