...
//        mpuCom.println("sd: "+ String(sensor_direction) + " d:" + String(direction));
    ...
    bool Setpoint(uint32_t delta_time)
    {
        ...
        {
            // mpuCom.println("SETPOINT " + String(channel) + " cr:" + String(currentTime) + " sensor:" + String(sensor) + " " + String(detecting) + " " + String(working) + " " + String(state));
    ...
    void UpdateFromRom()
    {
        ArrayTimerCopy(rom_channel[channel - 1].setpoint.timer_list, timer_list, 3);
        timer_size = rom_channel[channel - 1].setpoint.timer_size;
        timer_flag = rom_channel[channel - 1].setpoint.timer_flag;

        // temp = 3
        sensor_condition = rom_channel[channel - 1].setpoint.sensor_condition;
        sensor_direction = rom_channel[channel - 1].setpoint.sensor_direction;
        sensor_setpoint = rom_channel[channel - 1].setpoint.sensor_setpoint;
        sensor_flag = rom_channel[channel - 1].setpoint.sensor_flag;

        //co2 = 5
        sensor = rom_channel[channel - 1].setpoint.sensor;
        direction = rom_channel[channel - 1].setpoint.direction;
